...
(function initSupabase() {
  ...
  if (!isPlaceholder && libAvailable) {
    try {
      ...
      console.error("[Patinhas] Não foi possível iniciar o Supabase, usando modo demonstração.", err);
      window.DEMO_MODE = true;
    }
  } else {
    if (!libAvailable) {
      console.warn("[Patinhas] Biblioteca do Supabase não carregou — modo demonstração ativo.");
    } else {
      console.info("[Patinhas] Chaves do Supabase não configuradas em js/config.js — modo demonstração ativo.");
    }
    window.DEMO_MODE = true;
  }
})();
